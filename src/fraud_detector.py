...
class FraudDetector:
    ...
    def compute_node_risk_scores(self, risk_weights_bundle: Optional[Dict] = None) -> Dict[str, float]:
        """Compute composite risk score for each node.

        T2.10: When a trained risk-weights bundle is passed (from
        `src.risk_score_learner.load_risk_weights`), uses the learned
        LogisticRegression instead of the hand-tuned weighted sum. Falls
        back to the hand-tuned weights when no bundle is available.
        ...
        """
        # ── Learned path (T2.10) ──────────────────────────────────────────
        if risk_weights_bundle:
            try:
                from risk_score_learner import score_nodes
                scores = score_nodes(self.graph, risk_weights_bundle)
                if scores:
